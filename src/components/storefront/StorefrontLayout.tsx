      ...
      <footer className="bg-gray-900 text-white" style={cfg.footer_bg_color ? { backgroundColor: cfg.footer_bg_color, color: cfg.footer_text_color ?? '#ffffff' } : undefined}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            <div>
              ...
              <div className="flex items-center gap-4">
                {cfg.facebook_url && <a href={cfg.facebook_url} target="_blank" rel="noopener noreferrer" className="text-white/40 hover:text-white transition-colors"><Facebook className="h-5 w-5" /></a>}
                {cfg.twitter_url && <a href={cfg.twitter_url} target="_blank" rel="noopener noreferrer" className="text-white/40 hover:text-white transition-colors"><Twitter className="h-5 w-5" /></a>}
                {cfg.instagram_url && <a href={cfg.instagram_url} target="_blank" rel="noopener noreferrer" className="text-white/40 hover:text-white transition-colors"><Instagram className="h-5 w-5" /></a>}
                {cfg.whatsapp_number && <a href={`https://wa.me/${encodeURIComponent(cfg.whatsapp_number.replace(/[^0-9+]/g, ''))}`} target="_blank" rel="noopener noreferrer" className="text-white/40 hover:text-white transition-colors"><MessageCircle className="h-5 w-5" /></a>}
                {!cfg.facebook_url && !cfg.twitter_url && !cfg.instagram_url && !cfg.whatsapp_number && (
                  <>
                    <span className="text-white/20"><Facebook className="h-5 w-5" /></span>
                    <span className="text-white/20"><Twitter className="h-5 w-5" /></span>
                    <span className="text-white/20"><Instagram className="h-5 w-5" /></span>
                  </>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              {agency.domain && (
                <p className="text-xs text-white/30 mt-4">🌐 {agency.domain}</p>
              )}
            </div>
          </div>
          <div className="border-t border-white/10 mt-10 pt-6 flex items-center justify-between">
            ...
            <p className="text-xs text-white/40">© {new Date().getFullYear()} {agency.name}. All Rights Reserved.</p>
          </div>
        </div>
      </footer>
    </div>
